...
WITH season_2017 AS (
  ...
  FROM {{ source('fbref_historic', '2017') }} A 
  LEFT JOIN {{ source('static_tables', 'historic_pl_tables') }} B 
    ON A.TEAM = B.TEAM AND B.SEASON_END_YEAR = 2018
  LEFT JOIN {{ source('fpl_historic', '2017') }} C 
    ON C.NAME LIKE CONCAT("%", A.NAME, "%") OR A.NAME LIKE CONCAT("%", C.NAME, "%")
  LEFT JOIN {{ source('fpl_historic', '2018') }} D 
    ON D.NAME LIKE CONCAT("%", A.NAME, "%") OR A.NAME LIKE CONCAT("%", D.NAME, "%")
)
SELECT * FROM season_2017;