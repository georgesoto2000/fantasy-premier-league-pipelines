...
WITH season_2018 AS (
  ...
  FROM {{ source('fbref_historic', '2018') }} A 
  LEFT JOIN {{ source('static_tables', 'historic_pl_tables') }} B 
    ON A.TEAM = B.TEAM AND B.SEASON_END_YEAR = 2019
  LEFT JOIN {{ source('fpl_historic', '2018') }} C 
    ON C.NAME LIKE CONCAT("%", A.NAME, "%") OR A.NAME LIKE CONCAT("%", C.NAME, "%")
  LEFT JOIN {{ source('fpl_historic', '2019') }} D 
    ON D.NAME LIKE CONCAT("%", A.NAME, "%") OR A.NAME LIKE CONCAT("%", D.NAME, "%")
)
SELECT * FROM season_2018;